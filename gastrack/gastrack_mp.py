...
class GasTrack:
    ...
    def __init__(self, input_dir, output_dir, width=(0.8, 'Mpc'),
        ...
        # image size
        self.imgsize = (3200, 3200)
        # self.imgsize = buffsize
        # unit_base
    ...
    def get_halo_map(self, ds, figId=None, filename=None):
        ...
        if self.save == True:
            exec("pz" + str(figId) +
                 "= yt.ProjectionPlot(ds, self.axis, self.halo_field, width=self.width)")
            # pz.set_buff_size(self.buff_size)
            filepath = os.path.join(self.output_dir, filename)
            exec("pz" + str(figId) + ".save(filepath)")

        return halo_map_norm

    def get_gas_map(self, ds, peaklist, figId=None, filename=None):
        ...
        # Generate gas map
        exec("pz" + str(figId) +
             "= yt.ProjectionPlot(ds, self.axis, self.gas_field, width=self.width)")
        # Set buff_size
        # pz.set_buff_size(self.buff_size)
        # Markers
        exec("pz" + str(figId) +
             ".annotate_marker((peaklist[0, 1], peaklist[0, 2]),coord_system='plot',plot_args={'color': 'blue', 's': 500})")
        exec("pz" + str(figId) + ".annotate_marker((peaklist[1, 1], peaklist[1, 2]),marker= '+', coord_system='plot', plot_args={'color': 'red', 's': 300})")
        idx = re.findall(r'[0-9][0-9][0-9]', filename)
        i = int(idx[0])
        exec("pz" + str(figId) +
             ".annotate_text((-0.3, 0.3), '%.2f Gyr' % (i * 0.02), coord_system='plot')")
        if self.save == True:
            filepath = os.path.join(self.output_dir, filename)
            exec("pz" + str(figId) + ".save(filepath)")
...
def main(f):
    ...
    gt1 = GasTrack(input_dir,output_dir)
    gt2 = GasTrack(input_dir,output_dir)
    # Init sub particle hdft files
    ps = partsplit.PartSplit(input_dir, input_dir)
    ...
    width_old = gt.width
    # cluster1
    f_c1 = ('snap_%03d_c1.hdf5' % snapidx)
    ...
    # cluster2
    f_c2 = ('snap_%03d_c2.hdf5' % snapidx)
    ...
    gt.save = True
    gt.width = width_old
    figId = np.mod(snapidx, 2)
    ds = gt.load_hdf5(f)
    gt.get_halo_map(ds, figId, haloname)
    gt.get_gas_map(ds, peaklist, figId,  gasname)



def get_files(files, BeginIdx, EndIdx):
    """Extarct filenames satisfied indices"""
    filenames = []
    for f in files:
        if os.path.splitext(f)[-1] == '.hdf5':
            snapidx = re.findall(r'[0-9][0-9][0-9]', f)
            snapidx = int(snapidx[0])
            if snapidx >= BeginIdx and snapidx <= EndIdx:
                filenames.append(f)
...
if __name__ == "__main__":
    ...
    files.sort()
    # Get index
    filenames = get_files(files, BeginIdx=0, EndIdx=6)
    print(filenames)
